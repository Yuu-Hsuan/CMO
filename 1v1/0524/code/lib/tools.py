...
def cmo_steam_observation_file_to_xml(file_path: str) -> str | None:
    """
    Reads the CMO Steam observation file as binary, then decodes:
      1. 尝试用 UTF-8 解码
      2. 若失败，fallback 到 latin1（一对一映射所有字节，不会丢数据）
    最后从 JSON 里取出 "Comments" 字段当作 XML 返回。
    """
    # 1. 先以二进制模式打开，保证读到所有原始 bytes
    try:
    ...
    # 2. 解码：优先 UTF-8，失败再 latin1
    try:
        observation_file_contents = raw.decode('utf-8')
    except UnicodeDecodeError:
        observation_file_contents = raw.decode('latin1')
    ...
    # 4. 取出 Comments 字段（就是 XML 字串）
    return observation_file_json.get("Comments")
