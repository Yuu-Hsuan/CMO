...
class MyAgent(BaseAgent):
    ...
    def apply_action(self,
        ...
        # 2) action == 4 → 嘗試攻擊
        #    遍歷所有 contacts，匹配任何 FFG 分類 或 名字以 "FFG" 開頭 的目標
        target = None
        for c in contacts:
        ...
        if target and 'ID' in target:
            fn = next(f for f in vf if f.name == "auto_attack_contact")
            return fn.corresponding_def(ac.ID, target['ID'])

        # 3) 沒有目標或 action 不是 4，就不出任何命令
        return ""
    ...
    def train_on_episode(self):
        ...
        lambda_ext = 1.0                   # extrinsic 權重係數
        dr = intrinsic_rewards + lambda_ext * ext
