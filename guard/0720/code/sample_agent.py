...
class MyAgent(BaseAgent):
    ...
    def __init__(self, player_side, enemy_side=None, manager_unit_name=None):
        ...
        # --- reward 相關常數 -------------------------------------------------
        self.win_reward   = 100          # 勝利獎勵
        self.fail_reward  = -100         # 敗北懲罰
        self.reward_scale = 25           # 獎勵縮放因子
        self.max_return   = self.win_reward + self.args.enemy_num * 10   # ≈ 100(達港) + 7*10(殲敵) + 預留其他
        ...
        if action == 7:
            ...
            if weapon_id is None:      # ❌ 沒彈藥 → 派去備援巡邏
                return actlib.set_unit_to_mission(
                    unit_name   = ac.Name,
                    mission_name= "T_PatrolSCS3"      # 指定的備援任務
                )
