...
class MyAgent(BaseAgent):
    ...
    def __init__(self, player_side, enemy_side=None, manager_unit_name=None):
        ...
        class Args:
            ...
            def __init__(self):
                ...
                self.n_actions = 8            # 動作空間大小: 7個巡邏任務 + 1個主動攻擊
    ...
    def get_done(self, features, *_):
        """
        1) AOE 沉沒          → 結束
        2) AOE 距離台中港 < 3 km → 結束
        """
        # 取得 AOE 物件
        aoe = self.get_unit_info_from_observation(
            features, self.player_side, self.AOE_list[0])
        ...
        # 計算 AOE ↔ 港口 直線距離 (km)
        earth_r   = 6371.0
        lon_scale = np.cos(np.radians(aoe.Lat))
        dx = (self.TC_PORT_LON - aoe.Lon) * np.pi * earth_r * lon_scale / 180.0
        dy = (self.TC_PORT_LAT - aoe.Lat) * np.pi * earth_r / 180.0
        dist_km = np.sqrt(dx*dx + dy*dy)

        return dist_km < 15.0                 # ★ 門檻自行調整
    ...
    def action(self, features: Multi_Side_FeaturesFromSteam) -> str:
        ...
        for i in range(A):                      # A = self.args.n_agents
            ...
            if not features.contacts[self.player_side] or local_states[i][5] <= 0.0:
                mask[7] = False          # 禁用攻擊
            masks.append(mask)
        # -----------------------------------------------------------
    ...
    def get_reward(self, state: np.ndarray, next_state: np.ndarray, score: int) -> np.ndarray:
        ...
        max_return = self.win_reward + self.args.enemy_num * 20 +  100
    ...
    def apply_action(self, action: int, ac: Unit, features: Multi_Side_FeaturesFromSteam) -> str:
        ...
        if action == 7:
            # 場上沒敵 → 忽略
            if not features.contacts[self.player_side]:
                return ""
            ...
            for mount in ac.Mounts:
                if mount.Name not in ('Hsiung Feng II Quad', 'Hsiung Feng III Quad'):
                    continue
                if mount.Weapons and mount.Weapons[0].QuantRemaining > 0:
                    weapon_id = 1133 if mount.Name == 'Hsiung Feng III Quad' else 1934
                    break

            if weapon_id is None:      # ❌ 沒彈藥 → 派去備援巡邏
                return actlib.set_unit_to_mission(
                    unit_name   = ac.Name,
                    mission_name= "T_PatrolSCS3"      # 你想指定的備援任務
                )
