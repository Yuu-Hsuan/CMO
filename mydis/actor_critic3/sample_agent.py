...
class MyAgent(BaseAgent):
    ...
    def action(self, features: FeaturesFromSteam, VALID_FUNCTIONS: AvailableFunctions) -> str:
        ...
        with torch.no_grad():
            ...
            self.logger.debug(f"根據策略選擇動作: {action}")
    ...
    def train(self):
        ...
        # 計算 policy logits 和 entropy
        for t in range(self.batch_size):
            logits, self.worker_hidden = self.worker(
                states[t].unsqueeze(0), 
                self.worker_hidden, 
                current_goals[t].detach()
            )
            dist = Categorical(logits=logits)
        ...
        # Worker 更新 (Actor loss + entropy bonus)
        actor_loss = -(advantages.detach() * log_probs).mean()
        entropy_loss = -self.entropy_coef * entropies.mean()
        total_worker_loss = actor_loss + entropy_loss
        ...
        total_loss = manager_loss.item() + total_worker_loss.item()
