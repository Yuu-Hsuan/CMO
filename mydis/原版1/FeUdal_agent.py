...
#         return q, h, single_past_goals, batch_past_goals
...
class Feudal_WorkerAgent(nn.Module):
    ...
    def forward(self, inputs, worker_hidden, goal):
        ...
        # 計算 s_t (weighted state)
        # s_t = U_t * w_t  # element-wise multiplication
        # 計算 s_t (weighted state)矩陣乘法
        q = torch.bmm(U_reshaped, w_t_reshaped).squeeze(-1)
        # 生成 Q 值
        # q = self.q_network(s_t)
