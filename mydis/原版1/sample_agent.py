...
class MyAgent(BaseAgent):
    ...
    def action(self, features: FeaturesFromSteam, VALID_FUNCTIONS: AvailableFunctions) -> str:
        ...
        if self.prev_state is not None and self.prev_action is not None:
            ...
            if done or self.episode_step > self.max_episode_steps:
                ...
                # —— 一次性把本回合的記憶加進 completed_episodes  —— 
    ...
    def train(self):
        ...
        # 在 train() 內，計算 current_values、next_values
        ...
        # 獲取原始距離
        '''
        raw_distance = self.get_distance(states[0])
        
        # 距離正規化處理
        if isinstance(raw_distance, torch.Tensor) and raw_distance.item() > 1.0:
            # 使用固定值100.0進行正規化
            scale_factor = 100.0
        ...
        # ==== 新增代碼結束 ====
        '''
        # 直接使用原始 distance（normalized state 下的歐氏距離）
        ...
        # 距離可能是 torch.Tensor 或 float/np.float，先統一成 float
