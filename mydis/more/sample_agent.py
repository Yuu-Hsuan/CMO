...
class MyAgent(BaseAgent):
    ...
    def train_on_episode(self):
        ...
        # GAE for Worker (基於外在獎勵 ext_rewards)
        deltas_W = ext_rewards + self.gamma * next_values_W * (1 - dones) - values_W
        advantages_W = torch.zeros_like(ext_rewards, device=self.device)
        last_adv_W = 0.0
        for t in reversed(range(T)):
            last_adv_W = deltas_W[t] + self.gamma * self.gae_lambda * (1 - dones[t]) * last_adv_W
            advantages_W[t] = last_adv_W

        # --- Manager 部分（內在獎勵） ---
        # 正確做法：分別對 states 和 next_states 估值，再從下標 dilation 開始切片
        values_M = self.manager_critic(states).squeeze(-1)[dilation:]        # [T-dilation]
        next_values_M = self.manager_critic(next_states).squeeze(-1)[dilation:]   # [T-dilation]
        dones_M = dones[dilation:]                                         # [T-dilation]

        # 計算內在獎勵
        intrinsic_rewards = self.compute_intrinsic_rewards(states, raw_goals, dilation)  # [T-dilation]

        # 再算 deltas 與 GAE
        deltas_M = intrinsic_rewards + self.gamma * next_values_M * (1 - dones_M) - values_M
        advantages_M = torch.zeros_like(intrinsic_rewards, device=self.device)
        last_adv_M = 0.0
        ...
        # --- Worker: 逐步 unroll LSTM，沿時間步傳遞 hidden ---
        # 把 hidden state 拆成 h_w, c_w，不要跟 dilation 搞混
        h_w, c_w = self.worker.init_hidden()
        all_log_probs = []
        all_entropy = []
        for t in range(T):
            # 取出單步輸入與目標
            state_t = states[t].unsqueeze(0)       # [1, state_dim]
            goal_t  = raw_goals[t].unsqueeze(0)    # [1, k]
        ...
        # 計算 loss
        worker_loss = -(advantages_W.detach() * log_probs).mean()
        returns_W = advantages_W.detach() + values_W
        worker_value_loss = F.mse_loss(values_W, returns_W)
        worker_entropy_loss = -self.entropy_coef * entropy.mean()
        ...
        # 用 dilation（int）來做 slice
        cos_term = cos_term_full[dilation:]  # [T-dilation]
