...
class MyAgent(BaseAgent):
    ...
    def train_on_episode(self):
        ...
        # --- Manager 部分（內在獎勵） ---
        # 正確做法：分別對 states 和 next_states 估值，再從下標 c 開始切片
        values_M = self.manager_critic(states).squeeze(-1)[c:]        # [T-c]
        next_values_M = self.manager_critic(next_states).squeeze(-1)[c:]   # [T-c]
        dones_M = dones[c:]                                         # [T-c]

        # 計算內在獎勵
        intrinsic_rewards = self.compute_intrinsic_rewards(states, raw_goals, c)  # [T-c]
        ...
        # 重置 RNN 隱藏態並擴展到 batch size
        h0, c0 = self.worker.init_hidden()
        h0 = h0.expand(T, -1)
        c0 = c0.expand(T, -1)

        # Worker 更新
        logits, _ = self.worker(states, (h0, c0), raw_goals)
        dist = Categorical(logits=logits)
