...
class RunningNorm:
    ...
    def __call__(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # EMA 更新
        self.m = self.momentum * self.m + (1 - self.momentum) * b_mean
        self.v = self.momentum * self.v + (1 - self.momentum) * b_var
...
class MyAgent(BaseAgent):
    ...
    def get_ship_extrinsic_reward(self, prev_s, curr_s, done):
        """
        prev_s / curr_s : 皆為該艦 state 向量 (長度 6)
        外在獎勵規則：
            · 20.0 * Δd              —— 前進獎勵（主導項）
            · +3 / +0.25 ...       —— 到達與接近的 shaping
            · -5 / -2               —— 離 0.7 / 0.6 以上的懲罰
            · +2.0                  —— 全隊抵達一次性 Bonus
        """
        # ① 前進獎勵
        reward = 20.0 * (prev_s[0] - curr_s[0])         # Δd

        # ② shaping
        d = curr_s[0]
        if d < self.done_condition:      # 已抵達
            reward += 3.0
        elif d < 0.2:
            reward += 2.3
        elif d < 0.3:
    ...
    def compute_intrinsic_rewards(self, delta_states, raw_goals):
        """
        delta_states: [T-c, A, F] = s_{t+c} - s_t
        raw_goals   : [T-c, A, k] = goal_t  (在 t 時刻寫死)
        回傳 [T-c, A] 的 cosine intrinsic
        """
        T_prime, A, feat_dim = delta_states.shape
        k = raw_goals.size(-1)
        # `delta_states` is [T', A, F]. Flatten to [T'*A, F] for the fc layer.
        delta_emb = self.manager.delta_fc_local(delta_states.reshape(-1, feat_dim)).view(T_prime, A, k)
        cos = F.cosine_similarity(
            F.normalize(delta_emb, dim=-1),
    ...
    def action(self, features: FeaturesFromSteam, VALID_FUNCTIONS: AvailableFunctions) -> str:
        ...
        :return: 執行的動作命令（字串）
        """
        if self.waiting_after_reset:
            if self.get_unit_info_from_observation(features, self.manager_name) is None:
                return ""                  # 繼續 idle
            self.waiting_after_reset = False  # 艦已出現 → 解除等待
        self.episode_step += 1
        ...
        if ac is None:
            ...
            reset_block = self.get_reset_cmd(features)
            self.reset()                 # 只重置內部統計，不要回傳指令
            self.waiting_after_reset = True
            return reset_block           # 這一步只送重置指令，其他什麼都別送
        ...
        if hasattr(ac, 'DBID') and hasattr(ac, 'Type'):
            if self.ship_dbid is None:
                ...
                self.ship_dbid = ac.DBID
                if self.manager_name in self.init_snapshot['friendly']:
                    self.init_snapshot['friendly'][self.manager_name]['DBID'] = ac.DBID
        ...
        if enemy_unit:
            if self.enemy_dbid is None:
                ...
                self.enemy_dbid = enemy_unit.DBID
                if enemy_unit.Name in self.init_snapshot['enemy']:
                    self.init_snapshot['enemy'][enemy_unit.Name]['DBID'] = enemy_unit.DBID
        
        # 動態補充敵方單位到快照
        for u in enemy_units:
            if u.Name not in self.init_snapshot['enemy']:
                self.init_snapshot['enemy'][u.Name] = {
                    'ID': u.ID,
                    'Lat': float(u.Lat),
                    'Lon': float(u.Lon),
                    'DBID': u.DBID,
                    'Mission': ENEMY_MISSION,
                    'Side': u.Side
                }
        
        # --- after enemy_unit 更新區塊 ---------------------------
        for u in features.units:
            side_dict = (self.init_snapshot['friendly']
                         if u.Side == self.player_side else
                         self.init_snapshot['enemy'])
            if u.Name in side_dict and side_dict[u.Name]['DBID'] != u.DBID:
                self.logger.info(f"[DBID Sync] {u.Name}: {side_dict[u.Name]['DBID']} → {u.DBID}")
                side_dict[u.Name]['DBID'] = u.DBID
        ...
        if self.manager is None:
            ...
            self.opt_value = optim.Adam(
                ...
                lr=1e-3,                      # Critic 可以大 2~4 倍
                weight_decay=1e-5             # 也可不上 L2，視情況删掉
            )
            # ------------------------------------------------------
        # -------------------------------------------------------
        ...
        need_new_goal = (
            not hasattr(self.manager, "current_goal") or          # 第一次還沒宣告
            self.manager.current_goal is None or                  # 剛 reset 完
            (self.episode_step % self.args.manager_dilation == 0) # 每 c 步更新
        )

        if need_new_goal:
            goal_mat, (h_m, c_m), raw_goal = \
                self.manager(manager_state, self.manager_hidden[self.manager_name])
            self.prev_raw_goal = raw_goal # 只有產生新目標才更新
        else:
        ...
        if self.prev_states is not None and self.prev_action is not None:
            ...
            if success or timeout:
                ...
                self.reset()
                self.waiting_after_reset = True
                return reset_block           # 同上，提早 return
    ...
    def train_on_episode(self):
        ...
        # ───── ② 取出對齊後的外在回饋，再標準化 ─────
        ext_mgr_aligned = ext_rewards[c:].mean(dim=1)       # [T',]，行向平均
        ext_mgr_std     = self.ext_norm_mgr(ext_mgr_aligned)

        # ───── ③ 同步切割 v / nv / dones ─────
        v_m_aligned   = v_m.squeeze(-1)[c:]                 # [T',]
        nv_m_aligned  = nv_m.squeeze(-1)[c:]                # [T',]
        dones_aligned = dones[c:]                           # [T',]
        ...
        self.stats_logger.log_stat("real_intrinsic_reward", dr.mean().item(), self.total_steps)
    ...
    def get_reset_cmd(self, features: FeaturesFromSteam) -> str:
        buf = []

        # -- 刪除我方 & 敵方所有單位 (get_side_units 更可靠) ----------
        sides_to_clear = [self.player_side]
        if self.enemy_side:
            sides_to_clear.append(self.enemy_side)
            
        for side in sides_to_clear:
            side_units = features.get_side_units(side)
            if not side_units:
                continue
            for u in side_units:
                buf.append(delete_unit(side=side, unit_name=u.Name))

        # ------------------------------------------------------------------
        # 2) 按快照把友軍原名重生
        # ------------------------------------------------------------------
        for name, meta in self.init_snapshot['friendly'].items():
            buf.append(
                add_unit(
                    type='Ship',
                    unitname=name,
        ...
        # 3) 敵軍原名重生並立即掛巡邏任務
        # ------------------------------------------------------------------
        for name, meta in self.init_snapshot['enemy'].items():
            buf.append(
                add_unit(
                    type='Ship',
                    unitname=name,
