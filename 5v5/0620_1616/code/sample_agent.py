...
class MyAgent(BaseAgent):
    ...
    def get_ship_extrinsic_reward(self, prev_s, curr_s, done):
        ...
        """
        # ① 距離進步：越靠近目標越正
        dist_delta = 50.0 * (prev_s[0] - curr_s[0])

        # ② 是否抵達
        arrived = 10.0 if curr_s[0] < self.done_condition else 0.0

        # ③ 全隊抵達一次性 bonus（會加給所有艦；想只加一次可改寫）
        win_bonus = 20.0 if done else 0.0

        return float(dist_delta + arrived + win_bonus)
    ...
    def train_on_episode(self):
        ...
        self.args.entropy_coef = max(0.005, self.args.entropy_coef * 0.995)
        ...
        # ───── ② 取出對齊後的外在回饋，再標準化 ─────
        ext_mgr_aligned = ext_rewards[c:, idx_mgr]          # [T',]
