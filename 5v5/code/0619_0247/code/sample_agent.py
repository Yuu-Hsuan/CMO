...
class MyAgent(BaseAgent):
    ...
    def get_team_extrinsic_reward(self,
                                  prev_states: list[np.ndarray],
                                  curr_states: list[np.ndarray],
                                  done: bool) -> float:
        """
        新外在獎勵 = 距離差平均 + 抵達比例平均 + 全艦皆抵達 bonus
        回傳單一 scalar，再由呼叫端 broadcast 到 [A]。
        """
        # ① 距離差平均（距離只有 0~1 正規化，所以不需再除以 max）
        dist_prev = np.array([s[0] for s in prev_states], dtype=np.float32)  # shape [A]
        dist_curr = np.array([s[0] for s in curr_states], dtype=np.float32)
        avg_dist_delta = 200*(dist_prev - dist_curr).mean()                      # >0 ⇒ 前進

        # ② 抵達比例：到達門檻 self.done_condition (=0.1) 內算 1
        arrived_flags = (dist_curr < self.done_condition).astype(np.float32)
        avg_arrived   = arrived_flags.mean()                                 # 0~1

        # ③ 全艦抵達 bonus
        win_bonus = 1.0 if done else 0.0

        # ------------ 權重（可自行調整） ------------
        w1, w2, w3 = 1.0, 1.0, 1.0
        raw_reward = w1*avg_dist_delta + w2*avg_arrived + w3*win_bonus

        # 若你想跟舊版一樣做「縮放」再回傳，可保留這行；不想縮放就直接 return raw_reward
        # max_return = w1*1 + w2*1 + w3*1           # 理論上界
    ...
    def action(self, features: FeaturesFromSteam, VALID_FUNCTIONS: AvailableFunctions) -> str:
        ...
        # Episode 結束 = 全艦抵達
        done = len(self.arrived_ships) == len(self.friendly_order)
        ...
        if self.prev_states is not None and self.prev_action is not None:
            ext_reward_scalar = self.get_team_extrinsic_reward(
                self.prev_states,          # list[A,F]：前一步所有艦
                current_states,            # list[A,F]：現在所有艦
                done                       # bool：是否全艦抵達
            )
        ...
        for i, (name, st) in enumerate(zip(self.friendly_order, current_states)):
            ...
            unit = self.get_unit_info_from_observation(features, name)
            action_cmd += "\n" + self.apply_action(act, unit, contacts, VALID_FUNCTIONS)
    ...
    def train_on_episode(self):
        ...
        dists_each    = final_state[:, 0]               # 每艦 dist_norm
        avg_norm_dist = float(dists_each.mean())        # ▶︎ 全艦平均
        mgr_norm_dist = float(dists_each[
            self.friendly_order.index(self.manager_name)
        ])
        ...
        self.logger.info(
            f"Final normalized distance  Avg(all): {avg_norm_dist:.4f} | "
            f"Manager: {mgr_norm_dist:.4f}"
        )
