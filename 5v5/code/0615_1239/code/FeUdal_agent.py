...
class Feudal_ManagerAgent(nn.Module):
    ...
    def forward(self, x, hidden):
        ...
        # 將原始目標加入緩衝區（deque 會自動丟棄最舊元件，一旦超過 maxlen）
        self.goal_buffer.append(raw_goal)
        
        # 直接把緩衝區內最多 c 筆 raw_goal 堆疊相加（即便還沒累積到 c 筆，也能處理）
        goals = torch.stack(list(self.goal_buffer))  # shape = [<=c, 1, k]
        goal = goals.sum(dim=0)                     # shape = [1, k]
