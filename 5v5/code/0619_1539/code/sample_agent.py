...
class MyAgent(BaseAgent):
    ...
    def get_ship_extrinsic_reward(self, prev_s, curr_s, done):
        ...
        """
        # ① 距離進步：越靠近目標越正
        dist_delta = 200.0 * (prev_s[0] - curr_s[0])

        # ② 是否抵達
        arrived = 1.0 if curr_s[0] < self.done_condition else 0.0
    ...
    def train_on_episode(self):
        ...
        # -------- 0) 先更新熵係數 (退火) ------------
        # 每「結束一個 episode」才乘 0.995；最低 0.02
        self.args.entropy_coef = max(0.02, self.args.entropy_coef * 0.995)
        ...
        # 組合獎勵
        dr = 1.0 * intr + 100.0 * ext_rewards[self.args.manager_dilation:]  # [T-c, A]
