...
class MyAgent(BaseAgent):
    ...
    def __init__(self, player_side, enemy_side=None, manager_unit_name=None):
        ...
        self.max_episode_steps = 500       # 最大回合步數
    ...
    def update_actor_critic(self, traj: list[dict]) -> float:
        ...
        # ---------- 5. return / advantage ----------
        returns = reward_raw.flip(0).cumsum(0).flip(0)
        value_cat = torch.stack([val_mgr[:,0], val_wk.mean(-1)], dim=1) \
                         .unsqueeze(-1)       # shape [T,2,1] 只用來 debug
        advantage = returns - torch.cat([val_mgr[:,:1], val_wk], dim=1)
        ...
        loss_act_wk = -(logp_wk * advantage[:,1:]).mean()
        loss_val_wk = 0.5 * advantage[:,1:].pow(2).mean()

        # ---------- Manager loss ----------
        # ... 前面已算好的 logp_mgr、adv_mgr
        loss_act_mgr = -(logp_mgr * adv_mgr.detach()).mean()
        loss_val_mgr = 0.5 * adv_mgr.pow(2).mean()
