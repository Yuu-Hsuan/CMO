...
class MyAgent(BaseAgent):
    ...
    def __init__(self, player_side, enemy_side=None, manager_unit_name=None):
        ...
        class Args:
            ...
            def __init__(self):
                ...
                self.phi_scale = 1          # shaping 係數(先放小一點測試)
        ...
        self.prev_global_state_pb = None # 用於 potential-based shaping
    ...
    def action(self, features: Multi_Side_FeaturesFromSteam) -> str:
        ...
        # 儲存前一步的全局狀態用於 potential-based shaping
        old_global_state = self.prev_global_state_pb     # 可能是 None
        self.prev_global_state_pb = global_state.copy()  # 每步都更新
    ...
    # --- NEW ---
    def potential(self, global_state: np.ndarray) -> float:
        """
        Φ(s)：取『所有還活著友艦的**實際距離(km)**平均』的負值。
        global_state: 長度 45 的向量，前 5 個元素循環代表每艘船:
            [alive, dist_norm, sinθ, cosθ, mount_ratio]。
        """
        dists = []
        for i in range(self.args.n_agents):
            alive_flag = int(global_state[i*5])  # 0 或 1
            dist_norm = global_state[i*5 + 1]
            if alive_flag == 1:
                dists.append(dist_norm * self.max_distance)
    ...
    def get_reward(self, state: np.ndarray, next_state: np.ndarray, score: int, old_global_state: np.ndarray = None, global_state: np.ndarray = None) -> np.ndarray:
        """
        計算單個單位的獎勵值
        ...
        參數:
            ...
            next_state: 下一個狀態向量
            score: 當前遊戲分數
            old_global_state: 前一步的全局狀態（用於 potential-based shaping）-----------------------------------------
            global_state: 當前全局狀態（用於 potential-based shaping）-----------------------------------------
        ...
        if old_global_state is not None:
            phi_now  = self.potential(old_global_state)     # Φ(s)
            phi_next = self.potential(global_state)         # Φ(s')
            shaping  = self.args.gamma * phi_next - phi_now # γ Φ(s') − Φ(s)
            reward  += self.args.phi_scale * shaping        # ← 10 是建議 scale，與 +10~+20 同級
        else:
            shaping = 0.0  # 第一步時 old_global_state 為 None
        # --- Potential-based shaping -----------------------------------------------------------------------------
    ...
    def get_rewards(self,features: Multi_Side_FeaturesFromSteam, state: list[np.ndarray], next_state: list[np.ndarray], score: int, old_global_state: np.ndarray = None, global_state: np.ndarray = None) -> list[np.ndarray]:
        """
        為所有友方單位計算獎勵值
        ...
        for i, name in enumerate(self.friendly_info.order):
            ...
            if unit is None:
                ...
                reward = self.get_reward(state[i], next_state[i], score, old_global_state, global_state)
    ...
    def reset(self):
        ...
        self.prev_global_state_pb = None
        self.episode_step = 0
        self.done = False
