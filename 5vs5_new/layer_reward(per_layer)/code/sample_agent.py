...
class MyAgent(BaseAgent):
    ...
    def __init__(self, player_side, enemy_side=None, manager_unit_name=None):
        ...
        # --- Gradient-reward 狀態（新版 per-agent）---
        self.prev_flat_grad_list  = [None] * self.args.n_agents
        self.grad_ema_list        = [0.0]  * self.args.n_agents
        self.grad_step_list       = [0]    * self.args.n_agents
        self.latest_g_reward_vec  = [0.0]  * self.args.n_agents
        self.latest_g_reward_sum_vec = [0.0] * self.args.n_agents
        ...
        if self.prev_state is not None and self.prev_action is not None:
            ...
            if self.prev_goal_vec is not None and self.prev_global_state is not None:
                ...
                for a in range(self.args.n_agents):
                    γ = self.args.grad_coeff
                    warm_ratio = min(1.0, self.grad_step_list[a] / self.args.grad_warmup)
                    g_share = γ * warm_ratio * self.latest_g_reward_vec[a]          # 不用再 /n_agents
                    ...
                    t["r_grad_raw"] = float(self.latest_g_reward_vec[a])
                    ...
                    if self.episode_step % self.c_steps == 0:
                        ...
                        for t in self.seg_buffer:
                            if t["type"] == "wrk":
                                ...
                                g_share = γ * (self.latest_g_reward_sum_vec[aid] / self.c_steps)
                                ...
                                t["r_grad_raw"] = float(self.latest_g_reward_sum_vec[aid] / self.c_steps) # ★ 記段平均
                            else:                               # Manager
                                t["reward"] = r_ext_bar

                        self.ext_reward_sum = 0.0               # ➤ 清零，開始累下一段
                        self.latest_g_reward_sum_vec = [0.0] * self.args.n_agents  # ➤ 清零 G-reward 累積
    ...
    def _flatten_grad(self, aid: int | None = None) -> torch.Tensor:
        """
        aid=None  ➜ 返回整個網路梯度（保留給 Manager 用）
        aid=i     ➜ 只返回第 i 個 worker 專屬 critic-head 的梯度
        """
        g_vec = []
        if aid is None:
            modules = (self.manager, self.worker)
            for m in modules:
                for p in m.parameters():
                    if p.grad is not None:
                        g_vec.append(p.grad.view(-1))
        else:
            # 只抓 worker.critic_head{1,2}[aid] 的參數
            for head in (self.worker.critic_head1[aid],
                         self.worker.critic_head2[aid]):
    ...
    def _update_grad_reward_per_agent(self):
        """
        更新每艘 worker 的梯度獎勵；需在 optimizer.step() **之前** 呼叫
        """
        for aid in range(self.args.n_agents):
            flat_grad = self._flatten_grad(aid)                     # (n,)
            prev = self.prev_flat_grad_list[aid]
            delta = torch.norm(flat_grad - prev, p=2) if prev is not None else torch.zeros(1, device=self.device)

            # EMA
            β = self.args.grad_ema_beta
            self.grad_ema_list[aid] = β * self.grad_ema_list[aid] + (1-β) * delta.item()

            # warm-up 係數
            step = self.grad_step_list[aid] + 1
            warm_ratio = min(1.0, step / max(1, self.args.grad_warmup))
            g_raw = self.grad_ema_list[aid] * warm_ratio

            # tanh 壓縮
            g_r = math.tanh(0.1 * g_raw)
            self.latest_g_reward_vec[aid] = g_r
            self.latest_g_reward_sum_vec[aid] += g_r

            # 滾動狀態
            self.prev_flat_grad_list[aid] = flat_grad.clone()
            self.grad_step_list[aid] = step
    ...
    def update_actor_critic(self, traj: list[dict]) -> float:
        ...
        # ------ 改成 per-agent 版本 ------
        self._update_grad_reward_per_agent()
