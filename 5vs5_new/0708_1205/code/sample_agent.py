...
from module.batch_agent.FeUdal_agent import (
    Feudal_ManagerAgent, Feudal_WorkerAgent, FeUdalCritic
)
...
class MyAgent(BaseAgent):
    ...
    def __init__(self, player_side, enemy_side=None, manager_unit_name=None):
        ...
        class Args:
            ...
            def __init__(self):
                ...
                self.goal_dim = 4             # 目標向量維度（用於FeUdal網絡）
        ...
        self.eps_end = 0.1                 # 最終探索率
        self.eps_decay_steps = 100000      # 探索率衰減的步數
        
        self.done_condition = 0.1          # 任務完成的閾值條件
        ...
        self.critic = FeUdalCritic(self.global_input_size, self.args).to(self.device)
        ...
        self.max_episode_steps = 1500       # 最大回合步數
    ...
    def action(self, features: Multi_Side_FeaturesFromSteam) -> str:
        ...
        # -------- Manager 前傳 --------
        # 確保 hidden 的第二維 = B*A (=5)
        if self.manager_hidden[0].size(1) != self.args.n_agents:
            self.manager_hidden = self.manager.init_hidden(batch_size=1)   # (1,5,64)

        # 把一份 global_state 複製 A 份
        mg_feat = np.tile(global_state, (self.args.n_agents, 1))          # [A,45]
        mg_tensor = torch.tensor(mg_feat, dtype=torch.float32, device=self.device)\
                        .view(1, 1, self.args.n_agents, -1)            # [1,1,5,45]
        ...
        if (self.step_cnt % self.c_steps) == 0:
            goal_seq, logp_seq, val_seq, self.manager_hidden = \
                self.manager(mg_tensor, self.manager_hidden)
            self.goal_t = goal_seq.detach()
            self.seg_buffer.append({
                "type":   "mgr",
                "state_g": global_state.copy(),                 # Manager 用
                "state_l": None,                                # Worker 才用
                "goal":    self.goal_t.squeeze(0).squeeze(0)[0].cpu().numpy(),
                "action":  None,                                # Manager 無離散動作
                "logp":    float(logp_seq[0,0,0].item()),
                "value":   float(val_seq[0,0,0].item()),
        ...
        if (self.goal_t is None):
            goal_seq, logp_seq, val_seq, self.manager_hidden = self.manager(mg_tensor, self.manager_hidden)   # goal_seq shape [1,1,A,g_dim]
            self.goal_t = goal_seq.detach()             # [1,1,A,3]

            # 只取第 0 個 agent 的 logp/value 作代表（或平均亦可）
            logp_m = float(logp_seq[0,0,0].item())
            val_m  = float(val_seq[0,0,0].item())
        ...
        with torch.no_grad():
            logits, values, self.worker_hidden = self.worker(wk_tensor,
                                                     self.worker_hidden,
                                                     self.goal_t)
        ...
        for i in range(A):
            ...
            self.seg_buffer.append({
                ...
                "goal":    self.goal_t.squeeze(0).squeeze(0)[i, : self.args.state_dim_d]
                            .cpu().numpy(),
                "action":  act_i,
                "logp":    logp,
                "value":   v_i,
    ...
    def update_actor_critic(self, traj: list[dict]) -> float:
        ...
        mgr_in      = mgr_state[:,None,None,:].repeat(1,1,A,1)   # [T,1,A,45]
        h0m         = self.manager.init_hidden(batch_size=1)      # (1, 5, 64)
        ...
        # --------------- 3. 前向傳遞 --------------- #
        _, _, val_mgr, _ = self.manager(mgr_in, h0m)      # [T,1,A,1]
        val_mgr = val_mgr.squeeze(1).squeeze(-1)          # ➜ [T,A]

        logits_wk, val_wk, _ = self.worker(wk_in, h0w, goal_in)
        # logits_wk: [T,1,A,4] → 去掉 batch=1
        logits_wk = logits_wk.squeeze(1)                  # [T,A,4]
        val_wk    = val_wk.squeeze(1).squeeze(-1)         # [T,A]

        # ---------- 5. return / advantage ----------
        # --- Worker return ---
        gamma = self.gamma            # 0.99
        returns = torch.zeros_like(reward_raw)
        running = torch.zeros(self.args.n_agents+1, device=device)

        for t in reversed(range(T)):
            running = reward_raw[t] + gamma * running
            returns[t] = running

        # Advantage
        adv_raw      = returns - torch.cat([val_mgr[:,:1], val_wk], dim=1)   # [T,Ag]
        adv_wk_raw   = adv_raw[:,1:]                                         # Worker 原始
        adv_wk_std   = (adv_wk_raw - adv_wk_raw.mean()) / (adv_wk_raw.std()+1e-8)
        adv_mgr_raw  = adv_raw[:,0]                                          # Manager 原始

        # 1. 取出 Manager 的 log-p, value, reward
        logp_mgr   = torch.tensor([t["logp"] for t in traj if t["type"] == "mgr"], device=device)   # [T]
        value_mgr  = val_mgr[:, 0]          # [T]
        rewards_mgr= torch.tensor([t["reward"] for t in traj if t["type"] == "mgr"], device=device)         # [T]

        # 2. γ^c 折扣（c = self.c_steps，與 goal 更新頻率一致）
        gamma_c = self.gamma ** self.c_steps
        returns_mgr = torch.zeros_like(rewards_mgr)
        running = 0.0
        for t in reversed(range(len(returns_mgr))):
            running = rewards_mgr[t] + gamma_c * running
            returns_mgr[t] = running

        # 3. Advantage & loss
        adv_mgr       = returns_mgr - value_mgr.detach()
        loss_act_mgr  = -(logp_mgr * adv_mgr).mean()       # policy-gradient
        loss_val_mgr  = 0.5 * adv_mgr.pow(2).mean()        # value

        # ---------- 7. Worker policy + value ----------
        dist = torch.distributions.Categorical(logits=logits_wk)
        logp_wk = dist.log_prob(action_raw[:,1:])
        entropy = dist.entropy().mean()

        # -- policy --
        loss_act_wk = -(logp_wk * adv_wk_std.detach()).mean()

        # -- value: 先把幅度對齊 Manager --
        var_mgr  = adv_mgr_raw.pow(2).mean().detach()
        var_wk   = adv_wk_raw .pow(2).mean().detach()
        target_scale = torch.sqrt( (var_mgr + 1e-8) / (var_wk + 1e-8) )

        # --- ✱ 平滑更新 ✱ ---
        self.wk_scale_ema = (1 - self.wk_alpha) * self.wk_scale_ema + self.wk_alpha * target_scale.item()
        scale_wk = torch.tensor(self.wk_scale_ema, device=device, dtype=torch.float32)

        loss_val_wk = 0.5 * (scale_wk * adv_wk_raw).pow(2).mean()

        # ---------- Manager loss ----------
        # ... 前面已算好的 logp_mgr、adv_mgr
        loss_act_mgr = -(logp_mgr * adv_mgr_raw.detach()).mean()
        loss_val_mgr = 0.5 * adv_mgr_raw.pow(2).mean()

        # ← 在這裡印出各部分
        self.logger.info(
            f"loss_act_mgr={loss_act_mgr.item():.4f} | "
            f"loss_val_mgr={loss_val_mgr.item():.4f} | "
            f"loss_act_wk={loss_act_wk.item():.4f} | "
            f"loss_val_wk={loss_val_wk.item():.4f} | "
        ...
        self.logger.info(f"total_loss={total_loss.item():.4f}")
