...
class FleetController:
    def __init__(self, player_side: str, enemy_side: str, ship_names: list[str]):
        ...
        # ✨ 滾動緩衝區用於 5 回合平均統計
        self.fleet_step_history = []
        self.fleet_loss_history = []
        self.fleet_return_history = []
    ...
    def action(self, features):
        ...
        for ag in self.agents:
            ...
            # ❶ 沒收到 loss 時：直接記 0.0；讓統計照常進行
            if ag.last_episode_loss == 0.0:
                ag.last_episode_loss = (
                    ag.episode_loss_history[-1] if ag.episode_loss_history else 0.0
                )
        ...
        if len(self.agents) > 0:
            # ✨ 累積 5 回合統計值
            self.fleet_step_history.append(fleet_step)
            self.fleet_loss_history.append(fleet_loss)
            self.fleet_return_history.append(fleet_return)

            # ✨ 每滿 5 輪再記錄 log
            if len(self.fleet_step_history) >= 5:
                step_avg = float(np.mean(self.fleet_step_history))
                loss_avg = float(np.mean(self.fleet_loss_history))
                return_avg = float(np.mean(self.fleet_return_history))

                self.agents[0].stats_logger.log_stat("fleet_episode_step", step_avg, self.agents[0].total_steps)
                self.agents[0].stats_logger.log_stat("fleet_loss", loss_avg, self.agents[0].total_steps)
                self.agents[0].stats_logger.log_stat("fleet_return", return_avg, self.agents[0].total_steps)

                # 清空 buffer
                self.fleet_step_history.clear()
                self.fleet_loss_history.clear()
                self.fleet_return_history.clear()
