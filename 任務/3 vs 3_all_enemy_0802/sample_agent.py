...
class MyAgent(BaseAgent):
    ...
    def __init__(self, player_side, enemy_side=None, manager_unit_name=None):
        ...
        # 設置日誌記錄器
        # ➊ 建議改成「MyAgent.<艦名>」；若沒傳進艦名就標 Unknown
        unit_label = manager_unit_name or "UnknownUnit"
        self.logger = logging.getLogger(f"MyAgent.{unit_label}")
    ...
    def action(self, features: Multi_Side_FeaturesFromSteam) -> str:
        ...
        if self.prev_state is not None and self.prev_action is not None:
            ...
            if self.prev_goal_vec is not None and self.prev_global_state is not None:
                delta_full = global_state - self.prev_global_state               # (45,)
                delta_mat  = np.stack([
                    delta_full[aid*5 + 1 : aid*5 + 4]      # dist_norm, sinθ, cosθ
                    for aid in range(self.args.n_agents)
                ])                                         # (A, 3)
                
                g_vec      = torch.tensor(self.prev_goal_vec, device=self.device, dtype=torch.float32) # (A,3)
                d_vec      = torch.tensor(delta_mat, dtype=torch.float32,
                                          device=self.device)                    # (A, 3)
        ...
        if (len(self.seg_buffer) >= (self.args.n_agents + 1)
            ...
            delta_mat = np.stack([
                (global_state - self.prev_global_state)[aid*5+1 : aid*5+4]
                for aid in range(self.args.n_agents)
            ])                                        # shape (A,3)

            g_vec  = torch.tensor(self.prev_goal_vec[:, :3], device=self.device)
            d_vec  = torch.tensor(delta_mat,          device=self.device)
            cost   = float(torch.cosine_similarity(g_vec, d_vec, dim=-1).mean().item())
            ...
            if self.done or self.episode_step > self.max_episode_steps:
                ...
                if self.prev_goal_vec is not None and len(self.seg_buffer) > 0:
                    ...
                    delta_full = (global_state - self.prev_global_state).copy()
                    segment = {
    ...
    def get_reward(self, state: np.ndarray, next_state: np.ndarray, score: int) -> np.ndarray:
        ...
        max_return = self.win_reward +  100
