...
class Feudal_WorkerAgent(nn.Module):
    def __init__(self, input_shape, args):
        ...
        # 一個 worker 兩個 head → A 個 worker 就有 2*A 個 head
        self.critic_head1 = nn.ModuleList(
            [nn.Linear(args.worker_hidden_dim, 1) for _ in range(args.n_agents)]
        )
        self.critic_head2 = nn.ModuleList(
            [nn.Linear(args.worker_hidden_dim, 1) for _ in range(args.n_agents)]
        )
    ...
    def forward(self, inputs, worker_hidden, goal):
        ...
        # ------- Critic heads：每艘船各跑自己的一組 head -------
        H = self.args.worker_hidden_dim
        out_agents = out.contiguous().view(T, B, A, H)   # 先保證連續

        v1_list = []
        v2_list = []
        for a in range(A):
            # 取出第 a 艘船在 (T,B) 時序上的特徵，再展平成 (T*B, H)
            feat_a = out_agents[:, :, a, :].reshape(T * B, H)
            v1_list.append(self.critic_head1[a](feat_a))   # (T*B, 1)
            v2_list.append(self.critic_head2[a](feat_a))   # (T*B, 1)

        # 把 A 個 (T*B,1) 串成 (T*B, A) → 再還原 (T,B,A)
        v1 = torch.cat(v1_list, dim=1).view(T, B, A)
        v2 = torch.cat(v2_list, dim=1).view(T, B, A)
        v  = 0.5 * (v1 + v2)                   # 仍給主流程用平均值
