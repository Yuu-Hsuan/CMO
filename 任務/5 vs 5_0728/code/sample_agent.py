...
class MyAgent(BaseAgent):
    ...
    class FriendlyInfo:
        ...
        def __init__(self, player_side, manager_unit_name: str):
            ...
            self.order = [manager_unit_name]     # ★ 只有自己
            self.alive = {manager_unit_name: 1}  # ★ 只有自己
        ...
        def init_episode(self, features):
            ...
            參數:
                features: 當前環境的觀察資料
            """
            # order 已經在 __init__ 中設置為只包含 manager_unit_name
            # 只需要確保存活狀態正確初始化
    ...
    def __init__(self, player_side, enemy_side=None, manager_unit_name=None):
        ...
        # 定義兩種不同的輸入維度
        self.local_input_size = 7 + 4*self.args.enemy_num
        self.global_input_size = self.args.n_agents*5 + self.args.enemy_num*4         # 9
    ...
    def get_states(self, features: Multi_Side_FeaturesFromSteam) -> list[np.ndarray]:
        unit = self.get_unit_info_from_observation(features, self.player_side, self.manager_unit_name)
        if unit is None:
            return [np.zeros(self.local_input_size, dtype=np.float32)]
        else:
            return [self.get_state(features, unit)]
    ...
    def completed(self):
        ...
        return self.done or self.timed_out